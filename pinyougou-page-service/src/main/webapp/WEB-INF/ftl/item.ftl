...
<head>
	...
	<script>
		...
		{
			"id":${item.id?c},/*freemarker默认会自动给数字加分号分割*/
			"title":"${item.title!''}",
				"price":${item.price?c},
			"spec": ${item.spec}
		} ,
						...
						<#list specificationList as spec>
							<dl>
								...
								<#list spec.attributeValue as item>
									...
									<dd>
										<a href="javascript:;"
										   <#-- {{}}  是angular的语法，引用js定义的变量，-->
												<#--'${spec.attributeName}'  是freemarker的语法，加引号的原因是他在生成页面的时候会将数据传入，是一个字符串
												ng-click="selectSpecification('机身内存','32G')"
												如果不加引号前端页面会报错
												-->
